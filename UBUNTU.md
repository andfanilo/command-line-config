...
Docker Engine on native Linux — no VM, no VHDX, no disk bloat. Uses `overlay2` directly on the host filesystem, so `docker system prune` actually frees space.
...
# 4. Verify (after logging out and back in)
...
## Phase 4: Shell Configuration (Zsh)
